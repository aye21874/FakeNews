...
df_80 = fakeNews_df_transformed_false %>% 
      group_by(fakeNews_df_transformed_false$`State Info`) %>% 
      dplyr::summarise(n = n()) %>%
      arrange(desc(n)) 
...
df_81 = fakeNews_df_transformed_false %>%
        group_by(fakeNews_df_transformed_false$`State Info`, fakeNews_df_transformed_false$Speaker) %>% 
        dplyr::summarise(n = n()) %>%
        arrange(desc(n)) 
...
treemap::treemap(df_83,
       ...
       type="value",
       format.legend = list(scientific = FALSE, big.mark = " ")) + theme_ben()
```
