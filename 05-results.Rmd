...
df_f3 <- df_fakeNews %>%
        filter(df_fakeNews$`State Info` != "Virginia director, Coalition to Stop Gun Violence")
...
df_order$Panel <- c(rep(1:6, each = 11), c(6,6,6))
...
df_f4 %>%
ggplot(aes(fill = Label, y = Count, x = reorder(StateInfo,total) )) + 
  geom_bar(position="stack", stat="identity") + 
