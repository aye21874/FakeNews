...
```{r}
theme_fakeNews_rotate <- function(base_size = 14, ang=90, add=0) {
  theme_bw(base_size = base_size) %+replace%
    theme(
      ...
      # Les axes
      axis.title = element_text(size = 11+add, face = "bold", margin = margin(10,10,10,10)),
      axis.text.x = element_text(size = 11+add, face = "bold", margin = margin(10,10,10,10), angle=ang),
      axis.text.y = element_text(size = 11+add, face="bold", margin = margin(10,10,10,10)),
      axis.line = element_line(color = "black", arrow = arrow(length = unit(0.3, "lines"), type = "closed")),
      # La légende
      legend.title = element_text(size = 11+add, face = "bold"),
      legend.text = element_text(size = 11+add, face = "bold")
    )}

theme_fakeNews <- function(base_size = 14, add=0) {
  theme_bw(base_size = base_size) %+replace%
    theme(
      # L'ensemble de la figure
      plot.title = element_text(size = 25, face = "bold", margin = margin(10,10,10,10), hjust = 0.5),
      ...
      panel.border = element_blank(),
      # Les axes
      axis.title = element_text(size = 13+add, face = "bold", margin = margin(10,10,10,10)),
      axis.text.x = element_text(size = 13+add, face = "bold", margin = margin(10,10,10,10)),
      axis.text.y = element_text(size = 13+add, face="bold", margin = margin(10,10,10,10)),
      axis.line = element_line(color = "black", arrow = arrow(length = unit(0.3, "lines"), type = "closed")),
      # La légende
      legend.title = element_text(size = 13+add, face = "bold"),
      legend.text = element_text(size = 13+add, face = "bold")
    )}

theme_fakeNews_4 <- function(base_size = 14, ang=0) {
  theme_bw(base_size = base_size) %+replace%
    theme(
      # L'ensemble de la figure
      plot.title = element_text(size = 25, face = "bold", margin = margin(10,10,10,10), hjust = 0.5),
      ...
      panel.border = element_blank(),
      # Les axes
      axis.title = element_text(size = 23, face = "bold", margin = margin(10,10,10,10)),
      axis.text.x = element_text(size = 23, face = "bold", margin = margin(10,10,10,10), angle=ang),
      axis.text.y = element_text(size = 23, face="bold", margin = margin(10,10,10,10)),
      axis.line = element_line(color = "black", arrow = arrow(length = unit(0.3, "lines"), type = "closed")),
      # La légende
...
ggplot(df_10, aes(fill=Label, y=Count, x=SpeakerJobTitle)) +
  ...
  labs(fill='Label') +
  facet_wrap(~ThePartyAffiliation,  ncol=1) + theme_fakeNews_rotate()
...
ggplot(df_12, aes(fill=df_12$SpeakerJobTitle, y=df_12$Count, x=df_12$Subjects)) +
  ...
  theme_fakeNews(add=5) + coord_polar()
...
treemap::treemap(df_83,
       ...
       type="value",
       format.legend = list(scientific = FALSE, big.mark = " ")) + theme_fakeNews(add=12)
```
...
ggparcoord(df_63, columns =2:10, scale = "globalminmax", groupColumn = 1)  + theme_fakeNews_4(ang=90) + xlab("Venue used to spread Fake News") + ylab("Frequency of Fake News")
...
filt = c("Virginia director, Coalition to Stop Gun Violence", "Unknown", "unknown", "N/A", "Tex", "Russia", "Qatar", "China", "Virgiia", "PA - Pennsylvania", "None", "United Kingdom", "the United States", "Washington D.C.", "Washington state", "ohio")
df_f3 <- df_fakeNews %>%
...
df_order$Panel <- c(rep(1:6, each = 9))
df_order$Panel <- factor(df_order$Panel, levels = 1:6,
                   labels = c("300+ reports", "100-300 reports", "50-100 reports",
                              "25-50 reports", "3-25 reports", "1-3 reports"))
...
```{r}
filt = c("Virginia director, Coalition to Stop Gun Violence", "Unknown", "unknown", "N/A", "Tex", "Russia", "Qatar", "China", "Virgiia", "PA - Pennsylvania", "None", "United Kingdom", "the United States", "Washington D.C.", "Washington state", "Montana", "Tennesse", "Mississippi", "Virgina", "Maine", "Rhode island")
dfl <- df_f3
dfl <- dfl %>%
  filter(!(dfl$`State Info` %in% filt))
...
df %>%
  ggplot(aes(y=n, axis1=Venue, axis2=Label, axis3=PartyAffiliation, axis4=SpeakerJobTitle)) +
  ...
  scale_x_discrete(limits = c("Venue", "Label", "Party", "Job Title"), expand = c(.1, .1)) +
